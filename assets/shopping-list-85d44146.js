import{K as H,N as l}from"./modal-local-storage-f7df611e.js";const o={ShopUlEl:document.querySelector(".shopping-list-container"),ShopStubEl:document.querySelector(".shopping-list-container"),shopRemoveBtnEl:document.querySelector(".shopping-delete-btn")},a="/js_90_DeadMakar/assets/oooops-mob-1x-b0b9c805.png",x="/js_90_DeadMakar/assets/oooops-mob-2x-5c7a8c33.png",m="/js_90_DeadMakar/assets/oooops-tab-1x-aa5b2255.png",g="/js_90_DeadMakar/assets/oooops-tab-2x-6e7faaa3.png",e="data:image/png;base64,",s="data:image/png;base64,",t="data:image/png;base64,",r="data:image/png;base64,",d="/js_90_DeadMakar/assets/symbol-defs-c8f30ebc.svg#icon-delete";function Q(h){const n=h.map(A=>`<li class="shopping-item">
      <div class="shopping-list-item">
        
          <img
            class="shopping-book-img"
            src="${A.book_image}"
            alt="${A.title}"
            width="100"
            height="142"
          />
          

    <div class="shopping-item-container">
      <h3 class="shopping-book-title">${A.title}</h3>

      <p class="shopping-book-category-text">${A.list_name}</p>

      <p class="shopping-book-description">${A.description}</p>

      <p class="shopping-book-author">${A.author}</p>

      <div class="shopping-link-wrap">
        <a
          href="${A.buy_links[0].url}"
          target="_blank"
          rel="noopener noreferrer"
          class="shopping-link"
        >
          <picture class="picture-amazon">
            <source
              srcset="
                ${e},
                ${s}
              "
              media="(min-width: 1440px)"
            />
            <source
              srcset="
                ${e},
                ${s}
              "
              media="(min-width: 768px)"
            />
            <source
              srcset="
                ${e},
                ${s}
              "
              media="(max-width: 768px)"
            />
            <img
              class="shopping-img-amazon"
              src="${e}"
              alt="amazon"
              width="32"
              height="11"
            />
          </picture>
        </a>
        <a
          href="${A.buy_links[1].url}"
          target="_blank"
          rel="noopener noreferrer"
          class="shopping-link"
        >
          <picture class="picture-book">
            <source
              srcset="
              ${t}, 
              ${r}"
              media="(min-width: 1440px)"
            />
            <source
              srcset="
              ${t}, 
              ${r}"
              media="(min-width: 768px)"
            />
            <source
              srcset="
              ${t}, 
              ${r}"
              media="(max-width: 768px)"
            />
            <img
              class="shopping-img-book"
              src="${t}"
              alt="book"
              width="11"
              height="11"
            />
          </picture>
        </a>
      </div>
    </div>
  </div>
  <button class="shopping-delete-btn" type="button" data-id="${A._id}">
    <svg class="shopping-delete-btn-icon" width="16" height="16">
      <use href="${d}" ></use>
    </svg>
  </button>
</li>
`).join("");o.ShopUlEl.insertAdjacentHTML("beforeend",n)}function f(){const h=`<li class="shopping-error-wrap">
      <p class="shopping-error-text">
        This page is empty, add some books and proceed to order.
      </p>
      
        <picture>
          <source
            srcset="
              ${m},
              ${g}
            "
            media="(min-width: 1440px)"
          />
          <source
            srcset="
              ${m},
              ${g}
            "
            media="(min-width: 768px)"
          />
          <source
            srcset="
              ${a},
              ${x}
            "
            media="(max-width: 768px)"
          />
          <img
        class="shopping-error-img"
        src="${a}"
        alt="books"
        width="265"
        height="198"
      />
        </picture>
    </li>`;o.ShopStubEl.innerHTML=h}let i=JSON.parse(localStorage.getItem(H))||[];o.ShopUlEl.addEventListener("click",w);c();function c(){try{if(i.length===0){f(i);return}Q(i)}catch(h){console.error("Error:",h),l.Notify.failure("Sorry, there are no images matching your search query. Please try again.")}}function w(h){if(!h.target.classList.contains("shopping-delete-btn"))return;h.target.closest("li.shopping-item");const n=h.target.dataset.id,A=i.findIndex(p=>p._id===n);A!==-1&&(i.splice(A,1),localStorage.setItem(H,JSON.stringify(i)),o.ShopStubEl.innerHTML="",c())}
